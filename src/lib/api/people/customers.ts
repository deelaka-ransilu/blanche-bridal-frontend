...
export type CustomerMeasurementsResult =
  | { success: true; data: CustomerMeasurement[] }
  | { success: false; message: string; error?: string; fields?: Record<string, string> };

export async function getCustomerMeasurements(customerId: string): Promise<CustomerMeasurementsResult> {
  const token = await getToken();
  const result = await apiRequest<CustomerMeasurement[]>(
    `/api/employee/customers/${customerId}/measurements`,
    { method: "GET" },
    token
  );
  return result as unknown as CustomerMeasurementsResult;
}